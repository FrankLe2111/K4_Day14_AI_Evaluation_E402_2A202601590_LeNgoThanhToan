...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        """
        chunk_tokens = [_tokenize(chunk) for chunk in contexts]
        expected_tokens = _tokenize(expected)
        if not expected_tokens:
            return 1.0
        relevant = [len(chunk_token & expected_tokens) / len(expected_tokens) >= relevance_threshold
    ...
    def run_full_eval(
        ...
        failure_type = None if passed else (
            "hallucination" if faithfulness < 0.3 else
            "irrelevant" if relevance < 0.3 else
            "incomplete" if completeness < 0.3 else "off_topic")
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            result = evaluator.run_full_eval(agent_fn(pair.question), pair.question,
                                             pair.context, pair.expected_answer,
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        """
        total = len(results)
        average = lambda name: sum(getattr(r, name) for r in results) / total if total else 0.0
        def optional(name):
        ...
        return {
            ...
            "avg_completeness": average("completeness"), "avg_context_recall": optional("context_recall"),
            "avg_context_precision": optional("context_precision"),
            "failure_types": {kind: sum(r.failure_type == kind for r in results)
                              for kind in sorted({r.failure_type for r in results if r.failure_type})},
        }
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new = {n: sum(getattr(r, n) for r in new_results) / len(new_results) for n in names}
        old = {n: sum(getattr(r, n) for r in baseline_results) / len(baseline_results) for n in names}
        regressions = [n for n in names if old[n] - new[n] > 0.05]
        return {**{f"new_avg_{n}": new[n] for n in names},
                **{f"baseline_avg_{n}": old[n] for n in names},
...
class FailureAnalyzer:
    ...
    def categorize_failures(
        ...
        """
        return {kind: sum(f.failure_type == kind for f in failures)
                for kind in sorted({f.failure_type for f in failures if f.failure_type})}
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, 1):
            fix = suggestions[index - 1] if index <= len(suggestions) else "Review failure trace"
            rows.append(f"| F{index:03d} | {failure.failure_type or 'unknown'} | {self.find_root_cause(failure)} | {fix} | Open |")
        return "\n".join(rows)
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append("Add a grounding checker and omit unsupported claims.")
        if categories.get("irrelevant") or categories.get("off_topic"):
            suggestions.append("Improve intent routing and question-focused answer instructions.")
        if categories.get("incomplete"):
            suggestions.append("Improve evidence coverage and require every expected claim.")
        return suggestions or ["Inspect retrieved evidence and add regression cases.",
                               "Calibrate borderline answers with human labels.",
                               "Track metric trends in CI before deployment."]
